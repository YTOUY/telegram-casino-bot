...
async function spinWheel() {
    ...
    const totalRotations = 2 + Math.random() * 1.5; // 2-3.5 полных оборота для реалистичного вращения
    ...
    // Продолжительность вращения - 7 секунд для плавной остановки
    const spinDuration = 7000; // 7 секунд
    
    // Очень плавная easing функция с постепенным ускорением и очень плавным постепенным замедлением
    function smoothEase(t) {
        // Первые 15% времени - плавное ускорение (проходим 25% пути)
        // Следующие 15% времени - максимальная скорость (проходим 25% пути)
        // Последние 70% времени - очень плавное постепенное замедление (проходим 50% пути)
        if (t < 0.15) {
            // Плавное ускорение
            const localT = t / 0.15; // 0-1
            return 0.25 * (1 - Math.pow(1 - localT, 3)); // ease-out-cubic для плавного старта
        } else if (t < 0.3) {
            // Максимальная скорость (почти линейно)
            const localT = (t - 0.15) / 0.15; // 0-1
            return 0.25 + 0.25 * localT;
        } else {
            // Очень плавное постепенное замедление - большая часть времени (70%)
            const localT = (t - 0.3) / 0.7; // 0-1
            // Используем более реалистичную функцию для очень плавного замедления
            // Ease-out-expo для естественного постепенного замедления как у реального колеса
            // Формула: 1 - 2^(-10*t) для плавного замедления
            const expEase = 1 - Math.pow(2, -10 * localT);
            // Добавляем небольшую синусоидальную составляющую для еще более плавного эффекта
            const sineEase = Math.sin(localT * Math.PI / 2);
            // Комбинируем для максимально плавного замедления
            const combinedEase = (expEase * 0.7 + sineEase * 0.3);
            return 0.5 + 0.5 * combinedEase;
        }
    }

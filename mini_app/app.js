...
async function spinWheel() {
    ...
    const totalRotations = 0.5 + Math.random() * 0.3; // 0.5-0.8 полных оборотов для очень медленной прокрутки
    ...
    // Продолжительность вращения - 4 секунды
    ...
    function smoothEase(t) {
        // Первые 20% времени - плавное ускорение (проходим 30% пути)
        // Следующие 10% времени - максимальная скорость (проходим 20% пути)
        // Последние 70% времени - очень плавное постепенное замедление (проходим 50% пути)
        if (t < 0.2) {
            // Плавное ускорение
            const localT = t / 0.2; // 0-1
            return 0.3 * (1 - Math.pow(1 - localT, 2)); // ease-out-quad для плавного старта
        } else if (t < 0.3) {
            // Максимальная скорость (почти линейно)
            const localT = (t - 0.2) / 0.1; // 0-1
            return 0.3 + 0.2 * localT;
        } else {
            // Очень плавное постепенное замедление - большая часть времени (70%)
            const localT = (t - 0.3) / 0.7; // 0-1
            // Используем более мягкую функцию для очень плавного замедления
            // Комбинация ease-out-quad и синусоиды для естественного постепенного замедления
            const easeOut = 1 - Math.pow(1 - localT, 2);
            const sineEase = Math.sin(localT * Math.PI / 2);
            const combinedEase = (easeOut * 0.5 + sineEase * 0.5); // Смешиваем для более плавного эффекта

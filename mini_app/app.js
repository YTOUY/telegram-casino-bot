...
async function spinWheel() {
    ...
    const totalRotations = 0.8 + Math.random() * 0.4; // 0.8-1.2 полных оборотов для медленной прокрутки
    ...
    // Плавная easing функция с плавным ускорением и очень плавным замедлением
    function smoothEase(t) {
        // Используем ease-in-out-quart для плавного ускорения и замедления
        // Это даст медленное начало, быстрый разгон в середине, и очень плавное замедление в конце
        if (t < 0.5) {
            // Первая половина - плавное ускорение (ease-in)
            const localT = t * 2; // 0-1
            return 0.5 * Math.pow(localT, 4); // ease-in-quart
        } else {
            // Вторая половина - очень плавное замедление (ease-out)
            const localT = (t - 0.5) * 2; // 0-1
            return 0.5 + 0.5 * (1 - Math.pow(1 - localT, 5)); // ease-out-quint для очень плавного замедления
        }
    }
